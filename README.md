# 👋 Hello, I'm Cheryl Mbani

**Software Engineer | Data Enthusiast | Problem Solver**

I'm a passionate software engineer with a unique background in Geoinformation Systems, transitioning into full-stack development. I love transforming complex data into impactful digital solutions.

## 🛠️ Tech Stack
- **Languages:** Python, JavaScript, SQL, HTML5, CSS3
- **Frameworks & Libraries:** Flask, React
- **Databases:** PostgreSQL, SQLite
- **Tools:** Git, GitHub, VS Code, QGIS, ArcGIS
- **Currently Learning:** Advanced React, Node.js

## 📊 GitHub Stats
![Your GitHub stats](https://github-readme-stats.vercel.app/api?username=cherylmbani&show_icons=true&theme=radical)

## 🌟 Featured Projects
<!-- We'll update this with your best projects later -->
- [Project Atlas]() - Full-stack geospatial data visualization
- [DataViz Dashboard]() - Environmental data analysis tool  
- [TaskFlow App]() - Productivity management system

## 📫 Let's Connect
- **LinkedIn:** [https://www.linkedin.com/in/cheryl-mbani-533484334/](https://www.linkedin.com/in/cheryl-mbani-533484334/)
- **Email:** mbanicheryl45@gmail.com
- **Portfolio:** Coming Soon <!-- We'll build this next! -->

⚡ **Fun fact:** I combine my geospatial analysis background with software engineering to create data-driven solutions!

<!--
**cherylmbani/cherylmbani** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
